...
import { 
  Users, Activity, LayoutDashboard, UserCircle, 
  Plus, Dumbbell, Settings, ChevronRight
} from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const fabActions = [
    { name: 'Nueva Rutina', icon: <Dumbbell size={18} />, color: 'bg-primary' },
    { name: 'Nuevo Atleta', icon: <Users size={18} />, color: 'bg-green-500' },
    { name: 'Nuevo Ejercicio', icon: <Plus size={18} />, color: 'bg-purple-500' },
  ];
      ...
      <aside className="hidden lg:flex w-72 flex-col border-r border-white/5 p-8 sticky top-0 h-screen">
        ...
        <div className="mt-auto pt-8 border-t border-white/5">
          <div className="flex items-center space-x-3 p-4 bg-white/5 rounded-3xl">
            ...
            <Settings size={18} className="text-foreground/20" />
          </div>
        </div>
      </aside>
